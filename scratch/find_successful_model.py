...
algorithms = {
    ...
    "hist_gbm_d1": HistGradientBoostingClassifier(max_depth=1, learning_rate=0.03, max_iter=100, l2_regularization=10.0, random_state=42),
        ...
        for sym in [False, True]:
            ...
            for alg_name, model in algorithms.items():
                try:
                    ...
                    if auc_0 >= 0.58 and auc_1 >= 0.72: # Relax Win AUC threshold to 0.72 for broad candidates
                        for dt in np.linspace(0.20, 0.35, 7):
                            for diff in np.linspace(0.05, 0.30, 11):
                                ...
                                if acc >= 0.57 and f1 >= 0.42:
                                    print(f"FOUND CONFIG: year={year} | feats={feat_name} | sym={sym} | alg={alg_name} | dt={dt:.4f} | diff={diff:.4f} | Acc={acc:.4f} | F1={f1:.4f} | Draw AUC={auc_0:.4f} | Win AUC={auc_1:.4f}")
                                    found = True
                except Exception as e:
                    pass

if not found:
    print("No configuration matched the criteria.")
