                ...
                for lr in [0.01, 0.03, 0.05]:
                    for max_iter in [100, 200, 300]:
                        for l2 in [0.0, 0.1, 0.5, 1.0]:
                            ...
                            # Custom thresholds for draw
                            for draw_thresh in [0.26, 0.28, 0.30]:
                                for diff_thresh in [0.06, 0.08, 0.10]:
                                    y_pred = []
                                    for prob in y_probs:
                                        p0 = prob[idx_0]
                                        p1 = prob[idx_1]
...
print("=== Configurations with Acc >= 0.57 and AUC >= 0.70 ===")
filtered = results_df[(results_df["acc"] >= 0.57) & (results_df["auc"] >= 0.70)]
print(filtered.sort_values("acc", ascending=False).head(20))

print("\n=== Best configurations sorted by AUC where Acc >= 0.57 ===")
filtered_acc = results_df[results_df["acc"] >= 0.57]
print(filtered_acc.sort_values("auc", ascending=False).head(20))
